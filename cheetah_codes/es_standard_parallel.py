...
def gradascent(useParallel, theta0, filename, sigma=1, eta=1e-3, max_epoch=200, N=100):
  ...
  for i in range(max_epoch): 
    ...
    out_theta_file = "files/twin_theta_{}.txt".format(env_name)
...
if __name__ == '__main__':
    ...
    print("number of params: ",theta_dim)
    ...
    for k in tqdm.tqdm(range(num_seeds)):
        ...
        with open(outfile, "a") as f:
            f.write("Seed {}:\n".format(k))
        theta, accum_rewards = gradascent(useParallel, theta0, outfile, sigma=0.5, eta=1e-2, max_epoch=max_epoch, N=N)
